...
def simplify_clauses(clauses):
    ...
    while True:
        ...
        if c3 is None:
            c3 = plusminus(clauses)
        if c3 is None:
            c1, c2 = one_difference(clauses)
            c3 = none_filter(cancel_difference, c1, c2)
        if c3 is None:
            c1, c2 = subset(clauses)
...
def minisat_simplify(clauses):

    infilename = f'/tmp/tmp_cnf_in_{np.random.randint(10000)}.cnf'
    outfilename = f'/tmp/tmp_cnf_out_{np.random.randint(10000)}.cnf'
    CNF(from_clauses=clauses).to_file(infilename)
    subprocess.run(f'minisat {infilename} -dimacs={outfilename}',
                   stdout=subprocess.PIPE, shell=True)
    try:
        cnf = CNF(from_file=outfilename)
    except:
        return clauses
    subprocess.run(f'rm {infilename} {outfilename}', shell=True)
    return cnf.clauses
